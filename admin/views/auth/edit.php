...
<div class="so_main">
  ...
  <div class="form2" style="border:1px solid #D5D5D5">
  	<form method="post" action="<?php echo site_url('auth/doEdit');?>">
	<input type="hidden" name="id" value="<?php echo $id;?>" />
    <dl class="lineD">
      <dt class="t">用户组名称：</dt>
	  <dd><input type="text" name="groupname" id="groupname" class="input" value="<?php echo $authdata['groupname'];?>" /></dd>
    </dl>
			...
			<?php foreach($vs as $keyname => $item):?>
				...
				<dl class="lineD">
				  	...
				  	<dd>
						...
						<input data="fa<?php echo $ke;?>_son" type="checkbox" name="model[<?php echo $fmodel;?>][]" <?php if(isset($authdata['controller'][$fmodel]) && ($authdata['controller'][$fmodel]) && array_keys($authdata['controller'][$fmodel],$itemkey)) echo 'checked="checked"';?> class="check" value="<?php echo $itemkey;?>" id="<?php echo $fmodel . $itemkey;?>"><label for="<?php echo $fmodel . $itemkey;?>"><?php echo $itemname;?></label>
						<?php endforeach;?>
				  	</dd>
				</dl>
				<?php endif;?>
